...
export class FunctionStackService {
  ...
  async create(createFunctionStackDto: CreateFunctionStackDto) {
    const stack = await this.stackRepository.findOne({ where: { id: +createFunctionStackDto.stack_id } })
    const func = await this.functionRepository.findOne({ where: { id: +createFunctionStackDto.function_id } })
    ...
    if (!stack || !func) {
      ...
      return new ForbiddenException(`stack: ${funcStack.stack_id.title} for function:${funcStack.function_id.title} already exist`)
